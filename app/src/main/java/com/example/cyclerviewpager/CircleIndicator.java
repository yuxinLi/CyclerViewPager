...
public class CircleIndicator extends LinearLayout {
    ...
    public static String TAG = "CircleIndicator";
    ...
    /** 上一次 onPageSelected 返回的 position */
    ...
    private final ViewPager.OnPageChangeListener mInternalPageChangeListener = new ViewPager.OnPageChangeListener() {
        ...
        public void onPageSelected(int position) {
            Log.w(TAG , " === onPageSelected begin === , position = "+ position + " , mLastPosition = "+mLastPosition) ;
            if (mViewpager.getAdapter() == null || mViewpager.getAdapter().getCount() <= 0) {
                return;
            }
            // 这次Indicator 高亮的位置
            int currentPos = (position - 1 + mCount) % mCount ;
            // 上次Indicator 高亮的位置
            int lastPos = (mLastPosition - 1 + mCount)%mCount ;
            Log.d(TAG , " === Indicator 上一次高亮位置 lastPos= "+ lastPos +" , 这一次高亮位置 currentPos = "+ currentPos) ;

            if (currentPos != lastPos){ // Indicator 这一次高亮的位置 和上一次高亮的位置不一样
                View lastIndicator = getChildAt(lastPos);
                if (lastIndicator != null){
                    Log.d(TAG , " === 使Indicator 位置 "+ lastPos+ "  缩小 ") ;
                    lastIndicator.setBackgroundResource(mIndicatorUnselectedBackgroundResId);
                    if (mAnimatorIn.isRunning()) {
                        mAnimatorIn.end();
                        mAnimatorIn.cancel();
                    }
                    mAnimatorIn.setTarget(lastIndicator);
                    mAnimatorIn.start();
                }


                View selectedIndicator = getChildAt(currentPos);
                if (selectedIndicator != null) {
                    Log.d(TAG , " === 使Indicator 位置 "+ currentPos+ "  放大 ") ;
                ...
                Log.e(TAG , " 这一次高亮的位置 和上一次高亮的位置 一样 ， 不做处理 ") ;
            }
            mLastPosition = position;
            Log.w(TAG , " === onPageSelected end ") ;
        }
    ...
    private void createIndicators() {
        ...
        Log.e(TAG , " createIndicators === getAdapter.getCount = "+ count + " , IndicatorCount = "+ mCount );
